...
func (b *Bot) Bet(C string, U User) {
  if C == "!bet" {
    ...
    if 2 < len(split) {
      ...
      checkBet := Query("SELECT betsId FROM bets where betId = '"+betId+"' AND betUser = '"+U.username+"'")
      ...
      if ioldPoints >= ibetPoints {
        ...
        Insert("bets (betId, betUser, betPoints, betToWin)", "('"+betId+"', '"+U.username+"', '"+betPoints+"', '"+betToWin+"')")
  ...
  if C == "!addbet" && (U.mod == "true" || U.username == strings.ToLower(b.Channel)) {
  ...
  if C == "!endbet" && (U.mod == "true" || U.username == strings.ToLower(b.Channel)) {
    betId := Query("SELECT betId FROM bet WHERE betState = 0")
    if betId == "" {b.SendWhisper("There is currently no bet in progress.", U.username); return}
    split := strings.SplitAfter(U.message, " ")
    if 1 < len(split) {
      ...
      for i := 0; i < len(betUsers); i++ {
        if strings.ToLower(strings.TrimSpace(betToWins[i])) == strings.ToLower(strings.TrimSpace(split[1])) {
          ...
          Update("user", "points = points + "+strconv.Itoa(wPoints*2), "name", "'"+betUsers[i]+"'")
        }
      }
